...
async function findBestResponse(message: string): Promise<ChatResponse> {
  ...
  try {
    ...
    if (explainMatch) {
      ...
      try {
        console.log(`Looking up explanation for verse: ${verseRef}`);
        
        // First try to find a custom explanation
        const verseKey = verseRef.toLowerCase().replace(/\s+/g, ' ').replace(/^\s*(\d+)\s+/g, '$1 ');
        const verseExplanation = verseExplanations[verseKey as keyof typeof verseExplanations];
        
        if (verseExplanation) {
          console.log(`Found custom explanation for ${verseRef}`);
          return {
        ...
        console.log(`No custom explanation found for ${verseRef}, fetching from Bible API`);
        ...
        console.error('Error processing verse explanation:', {
          verseRef,
    ...
    if (verseRefMatch && verseRefMatch[1]) {
      const verseRef = verseRefMatch[1].trim();
      
      try {
        console.log(`Looking up verse reference: ${verseRef}`);
        ...
        for (const [key, value] of Object.entries(biblicalKnowledge)) {
          if (key.includes(verseKey)) {
            console.log(`Found in knowledge base: ${verseRef}`);
            return value;
          }
        }
        
        // If not found in knowledge base, try to fetch from API
        console.log(`Not found in knowledge base, fetching from Bible API: ${verseRef}`);
        const passage = await bibleService.getPassage(verseRef);
        if (passage?.text) {
          return {
            response: `**${verseRef} (${passage.version})**\n\n${passage.text}\n\n*Would you like me to explain this passage in more detail?*`,
    ...
    console.log('Checking for general knowledge matches');
    for (const [key, value] of Object.entries(biblicalKnowledge)) {
      if (lowerMessage.includes(key)) {
        console.log(`Found matching knowledge base entry for: ${key}`);
        return value;
      }
    }
    ...
    if (/how\s+(?:do|can|should)\s+i\s+(?:find|get|be)\s+a\s+(?:godly\s+)?(?:spouse|partner|husband|wife)/i.test(lowerMessage)) {
      console.log('Matched relationship advice pattern');
    ...
    if (/what(?:'s|\s+is|\s+does\s+the\s+bible\s+say\s+about)?\s+(?:pray|prayer)/i.test(lowerMessage)) {
      console.log('Matched prayer question pattern');
      const entry = biblicalKnowledge['how to pray'];
      if (entry) return entry;
    }
    
    const definitionMatch = lowerMessage.match(/(?:what(?:'s|\s+is)|define)\s+(?:biblical\s+)?(love|faith|hope|grace)/i);
    if (definitionMatch && definitionMatch[1]) {
      const term = definitionMatch[1].toLowerCase();
      console.log(`Matched definition query for: ${term}`);
      if (term === 'love') {
    ...
    console.log('No specific matches found, returning default response');
    return defaultResponse;
  } catch (error) {
    console.error('Unexpected error in findBestResponse:', {
      error: error instanceof Error ? error.message : 'Unknown error',
      stack: error instanceof Error ? error.stack : undefined,
